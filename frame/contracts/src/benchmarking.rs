...
benchmarks! {
	...
	seal_input {
		...
		let code = create_code::<T>(ModuleDefinition {
			...
			call_body: Some(body(vec![
				...
				Instruction::I32Const(buffer_size as i32), // value
	...
	seal_get_storage_per_kb {
		...
		let code = create_code::<T>(ModuleDefinition {
			...
			call_body: Some(body_repeated(API_BENCHMARK_BATCH_SIZE, &[
				// The out length gets overwritten every time we call the function.
				Instruction::I32Const(key_len as i32), // where to store
